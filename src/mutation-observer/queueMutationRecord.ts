 ...
 *
 * To queue a mutation record of type for target with one or more of (depends on type) name name, namespace namespace,
 * oldValue oldValue, addedNodes addedNodes, removedNodes removedNodes, previousSibling previousSibling, and nextSibling
...
export default function queueMutationRecord (type: string, target: Node, data: MutationRecordInit) {
	...
	// 3. For each node in nodes, and then for each registered observer (with registered observer’s options as options)
	...
	interestedObservers.forEach((observer, index) => {
		...
		// 4.2. If name and namespace are given, set record’s attributeName to name, and record’s attributeNamespace to
		// namespace.

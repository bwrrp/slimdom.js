...
export default abstract class Node {
	...
	/**
	 * Lazily initialized key to determine order between unrelated nodes.
	 */
	private _orderKey: number | null = null;
	...
	public compareDocumentPosition(other: Node): number {
		...
		if (isAttrNode(node2)) {
			...
			if (attr1 !== null && node1 !== null && node2 === node1) {
				...
				(node2 as Element).attributes.forEach((attr) => {
		...
		if (ancestors1[0] !== ancestors2[0]) {
			if (ancestors1[0]._orderKey === null) {
				ancestors1[0]._orderKey = Math.random();
			}
			if (ancestors2[0]._orderKey === null) {
				ancestors2[0]._orderKey = Math.random();
			}
			return (
				Node.DOCUMENT_POSITION_DISCONNECTED |
				Node.DOCUMENT_POSITION_IMPLEMENTATION_SPECIFIC |
				(ancestors1[0]._orderKey > ancestors2[0]._orderKey
					? Node.DOCUMENT_POSITION_FOLLOWING
					: Node.DOCUMENT_POSITION_PRECEDING)
			);
		}
		...
		const node1ContainsNode2 = firstDistinctAncestorIndex === ancestors1.length;
		const node2ContainsNode1 = firstDistinctAncestorIndex === ancestors2.length;

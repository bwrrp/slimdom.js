...
export default class SlimdomTreeAdapter implements parse5.AST.TreeAdapter {
	...
	createElement (tagName: string, namespaceURI: string, attrs: parse5.AST.Default.Attribute[]): parse5.AST.Element {
		...
		attrs.forEach(attr => {
			...
			const attribute = new Attr(this._globalDocument, undefinedAsNull(attr.namespace), undefinedAsNull(attr.prefix), attr.name, attr.value, element);
	...
	adoptAttributes (recipient: parse5.AST.Element, attrs: parse5.AST.Default.Attribute[]): void {
		...
		attrs.forEach(attr => {
			if (!element.hasAttributeNS(undefinedAsNull(attr.namespace), attr.name)) {
				element.setAttributeNS(undefinedAsNull(attr.namespace), qualifiedName(attr.namespace, attr.prefix, attr.name), attr.value);
			}
		});
	}

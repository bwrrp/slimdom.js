...
describe('XMLSerializer', () => {
	...
	it('can serialize a CDATASection', () => {
		chai.assert.equal(
			serializer.serializeToString(document.createCDATASection('test')),
			'<![CDATA[test]]>'
		);
		chai.assert.equal(
			serializer.serializeToString(document.createCDATASection('&notanentity;')),
			'<![CDATA[&notanentity;]]>'
		);
		chai.assert.equal(
			serializer.serializeToString(
			...
			'<![CDATA[<element><!-- not a comment --></element>]]>'
		);

		// As only CDataSection's constructor protects against the invalid ]]> character sequence,
		// certain DOM mutations could result in invalid XML being generated by the serializer (same
		// behavior seen in current browsers)
		const invalidCDataSection = document.createCDATASection('');
		invalidCDataSection.data = 'test]]><not-an-element>';
		chai.assert.equal(
			serializer.serializeToString(invalidCDataSection),
	...
	it('can serialize a Comment', () => {
		chai.assert.equal(
			serializer.serializeToString(document.createComment('test')),
			'<!--test-->'
		);
	});
	...
	it("doesn't generate a new prefix for namespaced attributes", () => {
		// TODO: this depends on a deviation from the spec (see
...
describe('serializeToWellFormedString', () => {
	...
	it("throws if given something that isn't a node", () => {
		chai.assert.throws(
			() => (slimdom as any).serializeToWellFormedString({ nodeType: 1 }),
			TypeError
		);
	});
	...
	it('throws if two attributes share the same local name and namespace', () => {
		const el = document.createElement('test');
		// It is not possible to create this situation using the DOM API, as any attempt to set a
		// namespaced attribute will check by localName / namespaceUri only, and any null-namespaced
		// attribute can not have a prefix. Use an internal function to forceably create the invalid
		// state.
